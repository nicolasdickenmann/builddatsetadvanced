...
public class Graph {
     ...
     */
    public boolean addNeighbor(int n1, int n2) {
     ...
     */
    public int findNeighborIdx(int n1, int n2) {
        for (int i = 0; i < adjacencyList[n1].size(); i++) {
     ...
     * @return True iff successfully added to graph
     */
    public boolean addBidirNeighbor(int n1, int n2, int cap) {
     ...
     */
    public boolean removeBidirNeighbor(int n1, int n2, int cap) {
     ...
     */
    public boolean removeBidirNeighbor(int n1, int n2) {
    ...
    /**
     * Verify whether the graph is acyclic or not.
    ...
    public boolean isAcyclic(){
        ...
        // Call DFS function in order to check if the graph component
        // contains cycle or not
        for (int u = 0; u < numNodes; u++) {
            if (!visited[u]) {
                if (!isAcyclic(visited, u, -1))
                    return false;
            }
        }
    ...
    private boolean isAcyclic(boolean [] visited, int node, int parentNode) {

        // Mark the node as visited
        visited[node] = true;

        for (int i = 0; i < adjacencyList[node].size(); i++){
            int neighbour = adjacencyList[node].elementAt(i).getLinkTo();

            if (!visited[neighbour]) {
                if (!isAcyclic(visited, neighbour, node))
                    return false;
