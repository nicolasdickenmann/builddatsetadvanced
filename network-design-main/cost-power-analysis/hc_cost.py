...
def hc_cost_single_rack(N,a):
	N = int(N)
	N_origin = N

	a = 256;#int(a)
	p = 1#k/10.0 #maybe change it?
	...
	if 1==1:
# this I use when adding "small" parts of the HC
#	while(max_hypercube > 1):
		...
		_N_l_c = nr_groups * ((group_size / 2) * group_dim)
		N_l_c += _N_l_c
		E_i += _N_l_c * el(1)
...
#		[max_hypercube,max_dim] = lower_power_of_two(N_r_level)
	...
	#print(">>>> HC: N_r: %d, N_l_c: %d, k: %d, p: %d, max_hypercube: %d, max_length: %d, max_dim: %d" % (N_r, N_l_c, k, p, max_hypercube, max_length, max_dim))
...
def hc_cost(N, N_1, N_2):
	...
	assert((N_l_c + N_g_c) == (2**(k_r - 1) * k_r))

	final_cost = C_r + C_el + C_op
	final_power = N*nic_power() + N_r*router_power(k)

	if(N >= N_1 and N <= N_2):	
		print(">>>>>>>>>>>> HC (N = %d, k = %d)" % (N, k))
		print("\tN: %d, N_r: %d, k: %d, k': %d, p: %d" % (N, N_r, k, k_r, p))
